 ...
 * back to the scene's broader caption/topic context, and finally to a
 * generic network glyph if nothing matches.
 */
function pickIconKey(params: ResolveVisualParams): string {
  const { subject = '', action = '', environment = '', caption = '', keyMessage = '', topic = '', voiceover = '' } = params;
  const scoped = `${subject} ${action} ${environment}`.toLowerCase();
  const broad = `${caption} ${keyMessage} ${topic} ${voiceover}`.toLowerCase();

  const rules: Array<[string, string[]]> = [
    ['code', ['code', 'koodh', 'software', 'barnaamij', 'app', 'developer', 'python', 'website']],
    ['shield', ['ammaan', 'security', 'hack', 'threat', 'privacy', 'sirdoon', 'difaac']],
    ['coin', ['lacag', 'money', 'bank', 'fintech', 'payment', 'zaad', 'evc', 'currency', 'dollar']],
    ['leaf', ['beer', 'farm', 'agri', 'dalag', 'waraab', 'crop', 'agriculture']],
    ['pulse', ['caafimaad', 'health', 'doctor', 'dhakhtar', 'hospital', 'bukaan', 'cudur', 'isbitaal', 'patient']],
    ['book', ['arday', 'student', 'school', 'education', 'waxbarasho', 'study', 'class', 'jaamacad', 'university']],
    ['robot', ['robot', 'drone', 'hardware', 'automation', 'machine', 'mishiin']],
    ['wave', ['luuqad', 'language', 'voice', 'cod', 'speak', 'hadal', 'dhawaaq', 'af-soomaali']],
    ['briefcase', ['shaqo', 'office', 'meeting', 'team', 'shaqaale', 'employee', 'kalkaal']],
    ['chart', ['ganacsi', 'business', 'growth', 'economy', 'market', 'suuq', 'data', 'horumar', 'koror']],
  ];
...
<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1920" viewBox="0 0 1080 1920">
  ...
  <g transform="translate(70, 1130)">
    ...
    <text x="44" y="58" fill="${theme.secondary}" font-size="21" font-family="system-ui, sans-serif" font-weight="800" letter-spacing="2">${safeSubject} — ${safeAction}</text>
