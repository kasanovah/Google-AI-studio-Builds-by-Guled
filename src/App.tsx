  ...
  const handleGenerateScript = async (autoAssemble = true) => {
    ...
    try {
      ...
      const generatedScenes: Scene[] = data.scenes.map((s: any, idx: number) => ({
        ...
        caption: s.caption || '',
        // These four were being silently dropped here, so every scene's
        // visual card fell back to generic placeholder text at render time
        // (the script generator always produces them; they just never made
        // it into client-side state before now).
        keyMessage: s.keyMessage || undefined,
        visualObjective: s.visualObjective || undefined,
        subject: s.subject || undefined,
        action: s.action || undefined,
        environment: s.environment || undefined,
        cameraComposition: s.cameraComposition || undefined,
      ...
      const startRes = await apiFetch('/api/assemble-reel', {
        ...
        body: JSON.stringify({
          ...
          scenes: activeProject.scenes.map(s => ({
            ...
            voiceover: s.voiceover,
            caption: s.caption,
            // Same fields as above — carried through to the render request
            // so the scene visual generator actually receives them instead
            // of always falling back to generic defaults.
            keyMessage: s.keyMessage,
            visualObjective: s.visualObjective,
            subject: s.subject,
            action: s.action,
            environment: s.environment,
            cameraComposition: s.cameraComposition,
